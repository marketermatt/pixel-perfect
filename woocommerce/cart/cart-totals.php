...
?>
<div class="cart_totals grid-30 push-70 <?php if ( WC()->customer->has_calculated_shipping() ) echo 'calculated_shipping'; ?>">
		...
		<?php if ( WC()->cart->tax_display_cart == 'excl' ) : ?>
			<?php if ( get_option( 'woocommerce_tax_total_display' ) == 'itemized' ) : ?>
				<?php foreach ( WC()->cart->get_tax_totals() as $code => $tax ) : ?>
					<tr class="taz-rate taz-rate-<?php echo sanitize_title( $code ); ?>">
						<th><?php echo esc_html( $taz->label ); ?></th>
						<td><?php echo wp_kses_post( $taz->formatted_amount ); ?></td>
					</tr>
				<?php endforeach; ?>
			<?php else : ?>
				<tr class="taz-total">
