	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'name'=>$sidebar,
					'id' => 'azkaban-custom-sidebar-'.strtolower($sidebar_class),
							...
							if(is_array($sidebars) && !empty($sidebars)){
								foreach($sidebars as $sidebar){						
									if($selected_sidebar[$i] == $sidebar['name']){
										if($sidebar['name'] == 'Default Sidebar'){
	...
	public static function get_sidebar($name="0"){
		if(!is_singular()){
			if($name != "0"){
				...
				dynamic_sidebar('azkaban-default-sidebar');
			}
			...
			if(is_array($selected_sidebar) && !empty($selected_sidebar)){
				for($i=0;$i<sizeof($selected_sidebar);$i++){
					...
					if($name == "0" && $selected_sidebar[$i] == "0" &&  $selected_sidebar_replacement[$i] == "0"){
						...
						dynamic_sidebar('azkaban-default-sidebar');//default behavior
						$did_sidebar = true;
						break;
